...
function refreshData(){
  ...
  document.getElementById('stakebutton').onclick=stake2;
  ...
  web3.eth.getAccounts(function (err, accounts) {
    ...
    tokenContract.methods.totalSupply().call().then(function(bal){
      document.getElementById('GFISCAMTotal').textContent=weiToDisplay(bal)
    })

    tokenContract.methods.balanceOf(addr).call().then(function(bal){
      document.getElementById('GFISCAMBalance').textContent=weiToDisplay(bal)
      //disable stake button here if balance smaller than amount in input
      var val=document.getElementById('GFISCAMStakeAmount').value
    ...
    tokenContract.methods.totalBurned().call().then(function(burned){
      document.getElementById('GFISCAMBurnedTotal').textContent=weiToDisplay(burned)
    })
    tokenContract.methods.totalBurned().call().then(function(burned){
      document.getElementById('burnedtotal').textContent=weiToDisplay(burned)
    })
    ...
    stakingContract.methods.stakedTotalSum().call().then(function(staked){
      document.getElementById('GFISCAMStaked').textContent=weiToDisplay(staked)
    })
    stakingContract.methods.amountStaked(addr).call().then(function(staked){
      document.getElementById('GFISCAMStakedYours').textContent=weiToDisplay(staked)
      //disable unstake button here if balance smaller than amount in input
      var val=document.getElementById('GFISCAMUnStakeAmount').value
      var inputAmount=web3.utils.toWei(val?val:"0",'ether')
    ...
    stakingContract.methods.DIVIDEND_RATE().call().then(function(divr){
      document.getElementById('GFISCAMDivRate').textContent=divr/10+'%'
    })
    stakingContract.methods.BURN_RATE().call().then(function(brn){
      stakingContract.methods.STAKE_DIV_FEE().call().then(function(sdf){
        document.getElementById('GFISCAMUnstakeRate').textContent=brn/5+sdf/5+'%'
      })
...
function unStake2(){
  ...
  let tospend=web3.utils.toWei(document.getElementById('GFISCAMUnStakeAmount').value,'ether')//document.getElementById('GFISCAMUnStakeAmount').value
  if(Number(tospend)>0){
      web3.eth.getAccounts(function (err, accounts) {
        address=accounts[0]
...
function stake2(){
  ...
  let tospend=web3.utils.toWei(document.getElementById('GFISCAMStakeAmount').value,'ether')//document.getElementById('GFISCAMStakeAmount').value
  if(Number(tospend)>0){
      web3.eth.getAccounts(function (err, accounts) {
        address=accounts[0]
